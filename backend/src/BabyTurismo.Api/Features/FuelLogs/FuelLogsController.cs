...
using BabyTurismo.Application.Fleet.Fuel.Commands;
using BabyTurismo.Application.Fleet.Fuel.Queries;
using MediatR;
using Microsoft.AspNetCore.Authorization;
using Microsoft.AspNetCore.Mvc;

namespace BabyTurismo.Api.Features.FuelLogs;
...
public sealed class FuelLogsController : BaseController
{
    ...
    public async Task<IActionResult> UploadReceipt(IFormFile file)
    {
        if (file == null || file.Length == 0)
            return BadRequest(BabyTurismo.Shared.Results.Error.Validation("File.Empty", "File cannot be empty."));

        var extension = Path.GetExtension(file.FileName).ToLowerInvariant();
        if (string.IsNullOrEmpty(extension) || !AllowedExtensions.Contains(extension))
            return BadRequest(BabyTurismo.Shared.Results.Error.Validation("File.InvalidExtension", "Only .jpg, .jpeg, .png and .pdf files are allowed."));

        if (file.Length > 10 * 1024 * 1024)
            return BadRequest(BabyTurismo.Shared.Results.Error.Validation("File.TooLarge", "File size must be less than 10MB."));
    ...
    {
        if (id != command.Id)
            return BadRequest(BabyTurismo.Shared.Results.Error.Validation("FuelLog.IdMismatch", "The ID in the URL must match the ID in the body."));
