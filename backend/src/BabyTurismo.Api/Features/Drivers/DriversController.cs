...
namespace BabyTurismo.Api.Features.Drivers;
...
public sealed class DriversController : BaseController
{
    ...
    {
        var query = new BabyTurismo.Application.Operations.Drivers.Queries.GetDriversQuery(page, pageSize, searchTerm, status);
    ...
    public async Task<IActionResult> GetDriverById(
        ...
        CancellationToken cancellationToken)
    {
        var result = await Mediator.Send(new BabyTurismo.Application.Operations.Drivers.Queries.GetDriverByIdQuery(id), cancellationToken);
        ...
        CancellationToken cancellationToken)
    {
        if (command.Id != Guid.Empty && id != command.Id)
            return BadRequest(BabyTurismo.Shared.Results.Error.Validation("Driver.IdMismatch", "The ID in the URL must match the ID in the body."));
